...
#SBATCH -o /pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/logs/fair_v3_%j.out
#SBATCH -e /pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/logs/fair_v3_%j.err
#SBATCH -J fair_v3
...
NUM_BLOCKS = 200  # 2GB per rank = 32GB total
...
# Generate data (keep in memory for both tests)
np.random.seed(42 + RANK)
data_blocks = [np.random.bytes(BLOCK_SIZE) for _ in range(NUM_BLOCKS)]

if RANK == 0:
    print(f"[RANK {RANK}] Generated {NUM_BLOCKS} blocks in memory")

###############################################################################
# TEST 1: LMCache style (per-file) - Write
###############################################################################
lmc_dir = f"{SCRATCH}/lmc_v3_{JOB_ID}/rank_{RANK:04d}"
...
if RANK == 0:
    print(f"[RANK 0] LMCache write done: {lmc_write_time:.2f}s")

# Sync and wait for Lustre
import time as tm
tm.sleep(2)
...
if RANK == 0:
    print(f"[RANK 0] LMCache cold read done: {lmc_read_time:.2f}s")

# Cleanup
shutil.rmtree(os.path.dirname(lmc_dir), ignore_errors=True)

###############################################################################
# TEST 2: Cascade style (aggregated + striped) - Write
###############################################################################
cascade_dir = f"{SCRATCH}/casc_v3_{JOB_ID}/rank_{RANK:04d}"
...
if RANK == 0:
    print(f"[RANK 0] Cascade write done: {cascade_write_time:.2f}s")
...
try:
    ...
    if RANK == 0:
        print(f"Warning drop cache agg: {e}")
...
if RANK == 0:
    print(f"[RANK 0] Cascade cold read done: {cascade_read_time:.2f}s")
...
if RANK == 0:
    print(f"\n[RANK 0] Per-Rank Bandwidth:")
    print(f"  LMCache:  Write {lmc_write_bw:.3f} GB/s, Read {lmc_read_bw:.3f} GB/s")
    print(f"  Cascade:  Write {cascade_write_bw:.3f} GB/s, Read {cascade_read_bw:.3f} GB/s")
...
if RANK == 0:
    ...
    out = f"{PROJECT_DIR}/benchmark/results/fair_v3_{JOB_ID}.json"
