...
echo "============================================"
echo "FAIR BENCHMARK V4 - Sequential operations"
...
if RANK == 0:
    print(f"\n{'='*60}")
    print("FAIR BENCHMARK V4: Sequential Lustre Operations")
...
###############################################################################
lmc_dir = f"{SCRATCH}/lmc_v4_{JOB_ID}/rank_{RANK:04d}"
...
comm.Barrier()
os.sync()
time.sleep(3)  # Wait for Lustre metadata
comm.Barrier()
...
for path in paths:
    if not os.path.exists(path):
        missing += 1
if missing > 0:
    print(f"[RANK {RANK}] ERROR: {missing} files missing!")
    sys.stdout.flush()
...
###############################################################################
cascade_dir = f"{SCRATCH}/casc_v4_{JOB_ID}/rank_{RANK:04d}"
...
if RANK == 0:
    ...
    print("\n" + "="*70)
    print("RESULTS - FAIR: Both from Lustre Cold Read")
    print("="*70)
    ...
    out = f"{PROJECT_DIR}/benchmark/results/fair_v4_{JOB_ID}.json"
