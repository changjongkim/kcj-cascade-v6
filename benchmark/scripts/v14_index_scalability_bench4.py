...
def run_index_scalability_benchmark():
    ...
    parser.add_argument("--scale-steps", type=int, nargs='+', default=[1000, 10000, 50000], help="List of scale steps in blocks")
    parser.add_argument("--disk-mode", action="store_true", help="Force Cascade to use Lustre primarily (0 GPU, 1GB RAM) for storage baseline comparison")
    ...
    if system_name.lower() == "cascade":
        if args.disk_mode:
            config = {
                ...
                "lustre_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/tmp/cascade_disk_{job_id}"
            }
        else:
            config = {
                ...
                "use_gpu": True, 
                "use_compression": False,
                "lustre_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/tmp/cascade_overflow_{job_id}"
            }
