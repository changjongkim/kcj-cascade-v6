...
#SBATCH -o /pscratch/sd/s/sgkim/Skim-cascade/benchmark/logs/cold_hot_5sys_%j.out
#SBATCH -e /pscratch/sd/s/sgkim/Skim-cascade/benchmark/logs/cold_hot_5sys_%j.err
#SBATCH -J cold_hot_5sys

###############################################################################
# COLD/HOT BENCHMARK - 5 System Comparison
# 
# Systems: Cascade, LMCache, PDC, Redis, HDF5
# Tests both hot (SHM/page cache) and cold (Lustre) read performance
###############################################################################
...
echo "============================================"
echo "COLD/HOT 5-SYSTEM BENCHMARK"
...
class CascadeStore:
    """Cascade with mmap SHM."""
    def __init__(self, shm_path: str, capacity_gb: float = 4.0):
        if not HAS_CASCADE_CPP:
            raise RuntimeError("cascade_cpp not available")
...
class HDF5Store:
    ...
    def get_cold(self, block_id: str) -> Tuple[Optional[bytes], float]:
        """For cold, reopen file after cache drop."""
        if block_id not in self.index:
            return None, 0.0
        self.h5file.close()
...
###############################################################################
# Aggregated Cascade Store (Lustre-backed)
###############################################################################
class CascadeAggStore:
...
def run_benchmark():
    if RANK == 0:
        print(f"\n{'='*70}")
        print(f"COLD/HOT BENCHMARK - 5 SYSTEMS")
    ...
    if HAS_CASCADE_CPP:
        ...
        shm_path = f"/dev/shm/cascade_{JOB_ID}_{RANK}"
        cascade = CascadeStore(shm_path, capacity_gb=4.0)
    ...
    if RANK == 0:
        ...
        result_file = f"{PROJECT_DIR}/benchmark/results/cold_hot_5sys_{JOB_ID}.json"
...
echo "[DONE] Cold/Hot 5-system benchmark completed."
