...
#SBATCH -N 4
#SBATCH -t 00:30:00
#SBATCH -J real_6sys_v2
#SBATCH --gpus-per-node=4
#SBATCH -o /pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/logs/real_6sys_v2_%j.out
#SBATCH -e /pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/logs/real_6sys_v2_%j.err

# ============================================================
# 실제 6개 시스템 비교 벤치마크 (v2 - /tmp 문제 수정)
...
#!/usr/bin/env python3
"""6개 시스템 비교 벤치마크: Cascade, vLLM, LMCache, PDC, Redis, HDF5"""
...
class VLLMStore:
    def __init__(self):
        ...
        try:
            ...
            if torch.cuda.is_available():
                self.torch = torch
                self.device = torch.device(f'cuda:{rank % 4}')
                self.available = True
                if rank == 0: print(f"  vLLM: GPU {torch.cuda.get_device_name()}")
        except: pass
    def put(self, key, data):
        if self.available:
            self.cache[key] = self.torch.from_numpy(np.frombuffer(data, dtype=np.uint8)).to(self.device)
        return len(data)
    def get(self, key):
    ...
    def cleanup(self):
        self.cache.clear()
        if self.available: self.torch.cuda.empty_cache()

# 3. LMCache (file-based, 세션별 저장 - LMCache 특성)
class LMCacheStore:
    def __init__(self):
...
class RedisStore:
    def __init__(self, port=16379):
        ...
        try:
            ...
            if rank == 0: print(f"  Redis: Not available ({e})")
    def put(self, key, data):
        if self.available: self.client.set(f"r{rank}:{key}", data)
        return len(data)
    def get(self, key):
        if self.available: return self.client.get(f"r{rank}:{key}")
    def cleanup(self):
        if self.available:
...
def main():
    ...
    for i, (name, StoreClass) in enumerate(systems, 1):
        if rank == 0:
            print(f"\n[{i}/6] {name}...")
        
        store = StoreClass() if name != "Redis" else StoreClass(port=16379)
    ...
    if rank == 0:
        ...
        for r in all_results:
            ...
            print(f"{r['system']:<12} {t:<15} {r.get('write_gbps',0):>12.2f} {r.get('read_gbps',0):>12.2f}")
        print("="*70)
