...
def main():
    ...
    if args.system.lower() == "cascade":
        config = {
            ...
            "lustre_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/cascade_bursty_{job_id}"
        }
        ...
        config = {"storage_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/lmcache_bursty_{job_id}"}
    elif args.system.lower() in ["llm-gpu", "vllm-gpu"]:
        config = {"storage_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/vllm_bursty_{job_id}"}
    elif args.system.lower() == "pdc":
        config = {"storage_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/pdc_bursty_{job_id}"}
    elif "hdf5" in args.system.lower():
        config = {"file_path": f"/pscratch/sd/s/sgkim/kcj/Cascade-kcj/benchmark/tmp/h5_bursty_{job_id}.h5", "use_mpi": True}
