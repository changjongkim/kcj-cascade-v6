...
Self-contained Slurm scripts and Python drivers for reproducing CASCADE
paper results. Each subdirectory maps to an AD task ($T_i$) and paper
figure/table.
...
Set these variables before submitting jobs:
...
Replace `#SBATCH -A <account>` with the reviewer's allocation in every
Slurm script.
...
- vLLM — APC and LMCache-backed modes (end-to-end inference and DeepCAM only)
- CASCADE — this work

## Notes

- Each Slurm `cd`s to `${REPO_ROOT}`, then invokes a driver via
  `script_example/<task>/<driver>.py`. Imports resolve via `PYTHONPATH`
  set in `setup_env.sh`.
- DeepCAM requires the MLPerf HPC benchmark dataset
  (https://github.com/mlcommons/hpc/tree/main/deepcam); stage the 512GB
  dataset per MLCommons instructions.
