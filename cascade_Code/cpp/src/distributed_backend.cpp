...
bool DistributedGPUBackend::put(const BlockId &id, const uint8_t *data,
  ...
  if (target_node == rank_) {
    ...
    // CRITICAL ISSUE: [DATA LOSS] Data is transferred to remote staging buffer,
    // but NO mechanism exists to trigger the remote node to move it to storage.
    // Data is effectively discarded/overwritten by the next request.
#ifdef USE_MPI
