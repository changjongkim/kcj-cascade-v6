...
Our experiments demonstrate that \Cascade achieves \textbf{5.7$\times$ higher write throughput} than LMCache and PDC, enabling efficient KV cache storage for HPC-scale LLM inference.
...
\begin{table}[t]
\centering
\caption{\textbf{Real C++ implementation benchmarks} on Perlmutter (4 nodes, 16 ranks, 16GB data, Job 48413611). 
\Cascade achieves \textbf{5.7$\times$ faster writes} than LMCache/PDC through SSE2 streaming stores and mmap optimization.}
...
\midrule
\rowcolor{green!15}
\textbf{\Cascade} & \textbf{5.01} & \textbf{80.14} & 3.58 & 57.25 & ShmBackend + Lustre \\
LMCache & 0.87 & 13.96 & 7.37 & 117.95 & local\_disk\_backend \\
PDC & 0.84 & 13.49 & 7.93 & 126.80 & pdc\_server \\
Redis & 0.10 & 1.59 & 0.14 & 2.20 & redis-server \\
HDF5 & 0.05 & 0.85 & 1.31 & 20.91 & h5py + gzip \\
\bottomrule
\end{tabular}
\end{table}
...
\addplot[fill=blue!70] coordinates {(Cascade,80.14) (LMCache,13.96) (PDC,13.49) (Redis,1.59) (HDF5,0.85)};
\end{axis}
\end{tikzpicture}
\caption{\textbf{Write throughput comparison (16 ranks, 4 nodes).} \Cascade achieves 80.14 GB/s total write throughput, which is \textbf{5.7$\times$ faster} than LMCache (13.96 GB/s) and PDC (13.49 GB/s).}
...
\textbf{\Cascade achieves 5.7$\times$ faster writes than LMCache/PDC.}
...
\begin{table}[t]
\centering
\caption{Summary: \Cascade vs. baselines on key metrics (4 nodes, 16 ranks, Job 48413611).}
\label{tab:summary}
...
\midrule
Write Throughput (Total) & 80.14 GB/s & 13.96 GB/s (LMCache) & \textbf{5.7$\times$} \\
Write Throughput (Per Rank) & 5.01 GB/s & 0.87 GB/s (LMCache) & \textbf{5.7$\times$} \\
Read Throughput (Total)* & 57.25 GB/s & 126.80 GB/s (PDC) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*,nosep]
    \item \textbf{5.7$\times$} higher write throughput than LMCache/PDC (80.14 vs 13.96 GB/s)
