...
\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{SHM vs Lustre Cold}: 7.9$\times$ faster (8.61 vs 1.09 GB/s)
    ...
    \item \textbf{Optimal Block Size}: 10MB for maximum throughput
\end{enumerate}

\textbf{Key Insight}: \Cascade's value comes from tiered caching with mmap SHM,
not from optimizing Lustre access. Hot tokens are served at memory speed (8.6 GB/s),
which is 7.9$\times$ faster than cold Lustre reads.
\end{tcolorbox}
