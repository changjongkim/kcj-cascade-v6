...
%==============================================================================
\subsection{All 5 Systems: Hot vs Cold Comparison}
\label{sec:lustre-comparison}
%==============================================================================

We evaluate all five systems on \textbf{16GB data} across 4 nodes (Job 48438725),
comparing both hot reads (SHM/page cache) and cold reads (\texttt{posix\_fadvise(DONTNEED)}).

\begin{table}[t]
\centering
\caption{\textbf{Full 5-system comparison} (Job 48438725; 4 nodes, 16 ranks, 16GB data).
\Cascade dominates on hot reads; per-file systems converge on cold reads.}
\label{tab:lustre-only}
\begin{tabular}{l|rrr|l}
\toprule
\textbf{System} & \textbf{Write} & \textbf{Hot Read} & \textbf{Cold Read} & \textbf{Storage Method} \\
 & \textbf{(GB/s)} & \textbf{(GB/s)} & \textbf{(GB/s)} & \\
\midrule
\rowcolor{green!15}
\textbf{\Cascade} & \textbf{44.99} & \textbf{363.53} & --- & mmap SHM (always hot) \\
PDC & 10.26 & 143.05 & \textbf{19.98} & Per-file + fsync \\
Redis & 10.22 & 141.54 & 20.01 & Per-file batch \\
LMCache & 10.79 & 137.14 & 19.76 & Per-file (baseline) \\
HDF5 & 0.85 & 28.71 & 1.10 & gzip compression \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Key Finding: Storage Format is Not the Differentiator.}
On cold Lustre reads, all per-file systems achieve similar throughput ($\sim$20 GB/s).
\textbf{\Cascade's value comes from mmap SHM tiering}: 363 GB/s hot reads, which is \textbf{18$\times$ faster than cold Lustre}.

\paragraph{Write Performance Analysis.}
\Cascade achieves \textbf{44.99 GB/s} write throughput, \textbf{4.2$\times$} faster than other systems ($\sim$10 GB/s).
This is because mmap writes are memory-speed operations, while per-file systems incur \texttt{fsync()} overhead.
...
\begin{table}[t]
\centering
\caption{\textbf{Hot vs Cold read performance} (Job 48438725; 4 nodes, 16 ranks, 16GB data).
Hot = data in SHM/page cache; Cold = Lustre with \texttt{posix\_fadvise(DONTNEED)}.}
\label{tab:hot-cold}
\begin{tabular}{l|rrr|l}
\toprule
\textbf{System} & \textbf{Write} & \textbf{Hot Read} & \textbf{Cold Read} & \textbf{Notes} \\
 & \textbf{(GB/s)} & \textbf{(GB/s)} & \textbf{(GB/s)} & \\
\midrule
\rowcolor{green!15}
\textbf{\Cascade} & \textbf{44.99} & \textbf{363.53} & --- & mmap SHM (always hot) \\
PDC & 10.26 & 143.05 & \textbf{19.98} & page cache (hot), per-file (cold) \\
Redis & 10.22 & 141.54 & 20.01 & page cache (hot), per-file (cold) \\
LMCache & 10.79 & 137.14 & 19.76 & OS page cache (hot), per-file (cold) \\
HDF5 & 0.85 & 28.71 & 1.10 & gzip compression overhead \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{Hot reads}: \Cascade leads with \textbf{363.53 GB/s} through direct mmap access---\textbf{2.65$\times$} faster than LMCache (137 GB/s)
    \item \textbf{Write throughput}: \Cascade achieves \textbf{44.99 GB/s}, \textbf{4.2$\times$} faster than other systems ($\sim$10 GB/s)
    \item \textbf{Cold reads}: All per-file systems converge to Lustre bandwidth ($\sim$20 GB/s)
    \item \textbf{HDF5 bottleneck}: gzip compression creates severe CPU overhead (28 GB/s hot, 1.1 GB/s cold)
\end{enumerate}
...
\begin{axis}[
    ...
    symbolic x coords={Cascade,PDC,Redis,LMCache,HDF5},
    xtick=data,
    ymin=0,
    ymax=400,
...
\addplot[fill=orange!70] coordinates {(Cascade,363.53) (PDC,143.05) (Redis,141.54) (LMCache,137.14) (HDF5,28.71)};
\addplot[fill=blue!70] coordinates {(Cascade,363.53) (PDC,19.98) (Redis,20.01) (LMCache,19.76) (HDF5,1.10)};
\legend{Hot (SHM/cache),Cold (Lustre)}
\end{axis}
\end{tikzpicture}
\caption{\textbf{Hot vs Cold read comparison} (Job 48438725). \Cascade achieves highest hot read (\textbf{363.53 GB/s}) through mmap SHM---\textbf{2.65$\times$} faster than LMCache. On cold reads, per-file systems converge to Lustre bandwidth ($\sim$20 GB/s).}
\label{fig:hot-cold}
\end{figure}
...
\Cascade's value comes from serving this hot data at \textbf{363.53 GB/s} (mmap SHM),
which is \textbf{18$\times$ faster than cold Lustre reads} (20 GB/s) and \textbf{2.65$\times$ faster than LMCache's page cache}.
\end{tcolorbox}
