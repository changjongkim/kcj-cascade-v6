...
To evaluate \Cascade's tiered architecture more comprehensively,
we conduct experiments where data volume \emph{exceeds} the SHM tier capacity,
forcing spillover to Lustre.

\begin{table}[t]
\centering
\caption{Tiered overflow benchmark scenarios (per rank).}
\label{tab:overflow-scenarios}
\begin{tabular}{l|r|r|r|r}
\toprule
\textbf{Scenario} & \textbf{Data} & \textbf{SHM Cap} & \textbf{Overflow} & \textbf{Description} \\
\midrule
All-fit & 500MB & 2GB & 0\% & Pure memory performance \\
75\% overflow & 2GB & 500MB & 75\% & Moderate spillover \\
95\% overflow & 5GB & 256MB & 95\% & Extreme spillover \\
\bottomrule
\end{tabular}
\end{table}

In the overflow scenarios, \Cascade's advantage comes from:
\begin{enumerate}[leftmargin=*,nosep]
    \item \textbf{Hot data in SHM}: Frequently accessed blocks remain in memory
    \item \textbf{Cold data on Lustre}: Less-accessed blocks spill to disk
    \item \textbf{Weighted throughput}: Effective bandwidth is weighted average of tiers
\end{enumerate}

The effective throughput model is:
\begin{equation}
    BW_{\text{effective}} = f_{\text{shm}} \cdot BW_{\text{shm}} + (1 - f_{\text{shm}}) \cdot BW_{\text{lustre}}
\end{equation}
where $f_{\text{shm}}$ is the fraction of data served from SHM.

For example, with 25\% in SHM and 75\% on Lustre:
\begin{itemize}[leftmargin=*,nosep]
    \item \Cascade: $0.25 \times 148 + 0.75 \times 2 = 38.5$ GB/s effective
    \item LMCache (all Lustre): $1.0 \times 2 = 2$ GB/s (cold read)
\end{itemize}

This demonstrates \Cascade's \textbf{tiered advantage}: even with 75\% overflow,
the SHM tier provides a 19$\times$ speedup over pure-Lustre solutions for hot data.

\begin{tcolorbox}[colback=blue!5,colframe=blue!50!black,title=Insight: Tiered Storage Value]
\Cascade's tiered architecture provides graceful degradation:
\begin{itemize}[nosep]
    \item \textbf{0\% overflow}: 148 GB/s (full memory speed)
    \item \textbf{50\% overflow}: 75 GB/s (still 37$\times$ faster than cold Lustre)
    \item \textbf{75\% overflow}: 38.5 GB/s (19$\times$ faster)
\end{itemize}
In contrast, single-tier systems like LMCache fall to Lustre speed immediately when data exceeds page cache.
\end{tcolorbox}
