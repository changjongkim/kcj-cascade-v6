...
## 📊 성능 결과 요약 (Job 48442166 실측)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     Storage Bandwidth Hierarchy (Measured)                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│ 🔥 GPU HBM (Hot Data in VRAM) - 512MB block                                 │
│     ██████████████████████████████████████████████████████████ 690.7 GB/s   │
│     └─ D2D cudaMemcpy (44% of 1555 GB/s HBM2e)                              │
│                                                                             │
│ ⚡ NVLink P2P (GPU-to-GPU)                                                  │
│     ████████████████████████████████████████████████████████████ 2570 GB/s  │
│     └─ GPU 1↔3 direct NVLink path (peak observed)                           │
│     ██████████ 85-90 GB/s  └─ Cross-socket via CPU (GPU 0↔1,2,3)            │
│                                                                             │
│ 🚀 Cascade (Hot SHM Read)                                                   │
│     █████████████████████████████████ 160.9 GB/s                            │
│     └─ C++ mmap + parallel memcpy (multi-block aggregate)                   │
│                                                                             │
│ 📥 PCIe H2D (CPU→GPU Write)                                                 │
│     ████████████ 26.8 GB/s                                                  │
│     └─ cudaMemcpyAsync with pinned memory (84% of 32 GB/s)                  │
│                                                                             │
│ 📤 PCIe D2H (GPU→CPU Read)                                                  │
│     ████████████ 26.3 GB/s                                                  │
│     └─ cudaMemcpy DeviceToHost (82% of 32 GB/s)                             │
│                                                                             │
│ 💾 Lustre ($SCRATCH)                                                        │
│     ▌ 0.96 GB/s                                                             │
│     └─ Parallel file system (per-client)                                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 🎯 핵심 성능 테이블 (Job 48442166)

| Tier | Operation | Measured | HW Limit | Efficiency |
|------|-----------|----------|----------|------------|
| **GPU HBM** | D2D | **690.7 GB/s** | 1555 GB/s | 44% |
| **NVLink** | P2P (peak) | **2570 GB/s** | 600 GB/s | 428%* |
| **Cascade SHM** | aggregate | **160.9 GB/s** | 200 GB/s | 80% |
| PCIe | H2D | 26.8 GB/s | 32 GB/s | 84% |
| PCIe | D2H | 26.3 GB/s | 32 GB/s | 82% |
| Lustre | write | 0.96 GB/s | ~5 GB/s | 19% |

*NVLink 2570 GB/s는 캐시 효과로 추정됨 (이론값 600 GB/s 초과)
